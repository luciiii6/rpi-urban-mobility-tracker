...
class Application:
    ...
    def preinitialization(self):
        ...
        if not os.path.exists(
            'output') and self.args.save_frames: os.makedirs('output')
        ...
        if self.args.live_view or self.args.save_frames: self.COLORS = (np.random.rand(32, 3) * 255).astype(int)
    ...
    def run(self):
        for i, pil_img in enumerate(self.img_generator(self.args)):
            ...
            # get detections
            detections = Utils.generate_detections(pil_img, self.interpreter, self.args.threshold)
            ...
            if len(detections) == 0: print('   > no detections...')
                ...
                for track in self.tracker.tracks:
                	...
                	if track.track_id not in self.counting_dict:
                		self.counting_dict[track.track_id] = [(bbox[0] + bbox[2])/2,(bbox[1]+bbox[3])/2]
                	else:
                		if self.counting_dict[track.track_id][1] < coord_count and (bbox[1]+bbox[3])/2 > coord_count:
                			ls = str(datetime.now()).split()
                			self.cursor.execute(f'INSERT INTO timestamp(time,date,direction) VALUES ("{ls[1]}","{ls[0]}",1);')
                			if 	self.args.placement == "facing":
                		...
                		if self.counting_dict[track.track_id][1] > coord_count and (bbox[1]+bbox[3])/2 < coord_count:
                			ls = str(datetime.now()).split()
                			self.cursor.execute(f'INSERT INTO timestamp(time,date,direction) VALUES ("{ls[1]}","{ls[0]}",0);')
                			if self.args.placement == "facing":
                				self.counter_out = self.counter_out + 1
                			else:
                				self.counter_in = self.counter_in + 1
            ...
            if self.args.live_view or self.args.save_frames:
                ...
                for track in self.tracker.tracks:
                    bbox = track.to_tlbr()                 
                    class_name = self.labels[track.id]
                    color = self.COLORS[int(track.track_id) % len(self.COLORS)].tolist()
                ...
                # persist frames
                if self.args.save_frames: cv2.imwrite(f'output/frame_{i}.jpg', cv2_img)
